...
## 3. Prezentacja działania i zrzuty ekranu (Przykłady dla 3 miast)

W celu zweryfikowania poprawności działania aplikacji w różnych warunkach klimatycznych, przeprowadzono sesję testową dla trzech polskich miast reprezentujących odmienne aury pogodowe:

### 3.1. Warszawa (Częściowe zachmurzenie, umiarkowana temperatura)
Po wpisaniu miasta **Warszawa** chatbot automatycznie pobiera dane pogodowe z API (lub bazy mock). W odpowiedzi generowana jest zrównoważona porada modowa na temperaturę 18°C. Dodatkowo model GPT AI w sekcji *AuraStyle AI* proponuje np. spacer po parku Łazienkowskim lub odpoczynek w kawiarni.

* **Zrzut ekranu:** `pokaz_dzialania/warszawa.png`

![Warszawa](pokaz_dzialania/warszawa.png)

### 3.2. Gdańsk (Sezon wietrzny, mżawka)
Dla miasta **Gdańsk** (temperatura 12°C, silny wiatr od morza i mżawka) silnik rekomendacyjny wskazuje na konieczność założenia kurtki przeciwwietrznej i czapki chroniącej uszy. Sztuczna inteligencja sugeruje natomiast np. odwiedzenie Muzeum Bursztynu lub spacer brzegiem morza w sztormiaku.

* **Zrzut ekranu:** `pokaz_dzialania/gdansk.png`

![Gdańsk](pokaz_dzialania/gdansk.png)

### 3.3. Zakopane (Zimowa aura, intensywny śnieg)
W przypadku **Zakopanego** (temperatura -2°C i intensywne opady śniegu) chatbot dobiera ciepłą kurtkę puchową, ocieplane buty zimowe, rękawiczki oraz szalik. Model GPT AI dodaje sugestię wypicia gorącej czekolady lub odwiedzenia schroniska górskiego.

* **Zrzut ekranu:** `pokaz_dzialania/zakopane.png`

![Zakopane](pokaz_dzialania/zakopane.png)

---

## 4. Podsumowanie zrealizowanych rozszerzeń funkcjonalnych (Sprawozdanie)
...
### 4.1. Uwierzytelnianie użytkownika i zarządzanie sesją
...
### 4.2. Nowoczesny Interfejs iOS Glassmorphism (High-Fidelity UI)
...
### 4.3. Subtelne Animacje Tła (Wiatraczek i Jaskółki)
...
### 4.4. Bezkluczykowa Integracja z API Pogodowym (Open-Meteo)
...
### 4.5. Integracja z Modelami GPT AI (Pollinations AI)
...
## 5. Publikacja i hosting (GitHub Pages)
...
### 5.1. Ścieżki Relatywne i Moduły ES6
...
### 5.2. Instrukcja uruchomienia GitHub Pages krok po kroku
...
## 6. Instrukcja uruchomienia lokalnego i dane testowe
...
### 6.2. Dane Logowania (Konto Testowe)
